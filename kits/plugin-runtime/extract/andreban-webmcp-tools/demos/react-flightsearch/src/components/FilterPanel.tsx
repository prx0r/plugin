...
export default function FilterPanel({
  ...
  const originAirports = [...new Set(flights.map((flight) => flight.origin))];
  ...
  const handleStopChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newStops = checked
      ...
      : filters.stops.filter((stop) => stop !== numValue);
  ...
  const handleOriginChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newOrigins = checked
      ...
      : filters.origins.filter((origin) => origin !== value);
  ...
  const handleDestinationChange = (
    ...
    const newDestinations = checked
      ...
      : filters.destinations.filter((destination) => destination !== value);
